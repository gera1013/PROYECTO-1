...
class Render(object):
    ...
    def glClear(self):
        if self.background:
            self.pixels = [[self.background.getColor(x / self.width, y / self.height) for x in range(self.width)] for y in range(self.height)]
        else:
    ...
    def glBaricentricTriangle(self, vA, vB, vC, textureCs, normals, _color_ = None):
